    ...
    IErrorResponseService errorResponseService) : ControllerBase
{
    ...
    {
        Result<Employee> result = await _employeeService
            .CreateEmployeeAsync(employeeForCreationRequest);

        if (result.IsSuccess)
        {
            var dto = result.Value!.ToEmployeeResponse(); // Use the extension here
        ...
        [FromBody] JsonPatchDocument<EmployeePatchRequest> patchDocument)
    {
        ...
        if (result.IsSuccess)
        {
            return Ok(result.Value!.ToEmployeeResponse()); // Use the extension here
        }
        ...
        [FromBody] EmployeeForUpdateRequest employeeForUpdateRequest)
    {
        ...
        Result<Employee> updatedEmployee = await _employeeService
            .UpdateEmployeeAsync(
        ...
        {
            return Ok(updatedEmployee.Value!.ToEmployeeResponse()); // Use the extension here
        }
...
//            Result<Employee> result = await _employeeService.GetEmployeeByIdAsync(employeeIdValidatedResult.Value!); // Assuming you have this method
//            return result.IsSuccess
//                ? Ok(_mapper.Map<EmployeeResponse>(result.Value!))
//                : HandleErrorResponse(result.ErrorType, result.Errors);
//        }
//        else
//        {
