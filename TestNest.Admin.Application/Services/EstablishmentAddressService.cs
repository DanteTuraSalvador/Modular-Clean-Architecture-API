    ...
    ILogger<EstablishmentAddressService> logger) : BaseService(unitOfWork, logger, exceptionHandlerFactory), IEstablishmentAddressService
{
    ...
    //                .SetNonPrimaryForEstablishmentAsync(establishmentIdResult.Value!, EstablishmentAddressId.Empty());
    ...
    //    catch (Exception ex)
    //    {
    //        _logger.LogError(ex, "Error creating establishment address.");
    //        return Result<EstablishmentAddress>.Failure(ErrorType.Internal, [new Error("ServiceError", ex.Message)]);
    //    }
    //}
    ...
    //           {
    //               return Result<EstablishmentAddress>.Failure(updateResult.ErrorType, updateResult.Errors);
    //           }

    //           Result<EstablishmentAddress> commitResult = await SafeCommitAsync(() => Result<EstablishmentAddress>.Success(updatedAddress));
    //           if (commitResult.IsSuccess)
    //           {
    //               scope.Complete();
    //               return commitResult;
    ...
    //       catch (Exception ex)
    //       {
    //           _logger.LogError(ex, "Error updating establishment address.");
    //           return Result<EstablishmentAddress>.Failure(ErrorType.Internal, new Error("ServiceError", ex.Message));
    //       }
    //   }
        ...
        EstablishmentAddressId establishmentAddressId)
    {
        ...
        {
            _logger.LogError(ex, "Error deleting establishment address.");
       ...
       EstablishmentAddressId? excludedAddressId = null)
    {
        ...
        if (exists)
        {
            return Result<bool>.Success(true); // Combination exists
        }

        return Result<bool>.Success(false); // Combination does not exist
    }
    ...
    //        Result<EstablishmentId> establishmentIdResult = IdHelper
    //            .ValidateAndCreateId<EstablishmentId>(establishmentAddressPatchRequest.EstablishmentId.ToString());
    //        if (!establishmentIdResult.IsSuccess)
    //        {
    //            return Result<EstablishmentAddress>.Failure(ErrorType.Validation, establishmentIdResult.Errors);
    //        }
    ...
    //        if (requestEstablishmentId != existingAddress.EstablishmentId)
    //        {
    //            return Result<EstablishmentAddress>.Failure(
    //                ErrorType.Unauthorized,
    //                new Error("Unauthorized", $"Cannot patch address. The provided EstablishmentId '{requestEstablishmentId}' does not match the existing address's EstablishmentId '{existingAddress.EstablishmentId}'."));
    //        }
    ...
    //                establishmentAddressPatchRequest.Latitude != default ? establishmentAddressPatchRequest.Latitude : existingAddress.Address.Latitude,
    //                establishmentAddressPatchRequest.Longitude != default ? establishmentAddressPatchRequest.Longitude : existingAddress.Address.Longitude
    //            );
    ...
    //        if (establishmentAddressPatchRequest.IsPrimary != existingAddress.IsPrimary)
    //        {
    //            if (establishmentAddressPatchRequest.IsPrimary)
    //            {
    //                Result setNonPrimaryResult = await _establishmentAddressRepository
    //                    .SetNonPrimaryForEstablishmentAsync(requestEstablishmentId, establishmentAddressId);
    ...
    //                }
    //            }
    //            updatedAddress = updatedAddress.WithIsPrimary(establishmentAddressPatchRequest.IsPrimary).Value!;
    //        }
    ...
    //    catch (Exception ex)
    //    {
    //        _logger.LogError(ex, "Error patching establishment address.");
    //        return Result<EstablishmentAddress>.Failure(ErrorType.Internal, [new Error("ServiceError", ex.Message)]);
    //    }
    //}
